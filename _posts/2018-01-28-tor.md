...
$$ verify(m,signature,KEY_(public))→\true\ \or\ \false $$
...
Tor has bandwidth rate limiting which allows it to limit the rate of bandwidth through a circuit. Because the Tor protocol outputs about the same number of bytes as it takes in, it is sufficient 
