...
A small message to all my university friends, I hate you for making me write this.
...
You can clearly see that in this graph for f(x^2 + 0) with a slider that goes from -10 to 10.

![Imgur](https://i.imgur.com/kvQojop.gif)

So in this image we have 2 functions. One function is a fancy \(f(x) = x^2\) and the other function is a straight line \(0x + m\).

The value m changes from 10 all the way through to -10. When the gradient of this second line is positive (where m > 0) all the lines (that's every single possible gradience from 0 through to 10) which are touching the curve at \(x, x^2\) are positive (they're above 0) and every similar line that has a negative gradience is negative (below 0). The line at exactly 0 gradience is the minimum.
